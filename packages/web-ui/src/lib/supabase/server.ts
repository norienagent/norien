 ...
 * handlers, and the auth callback. The session cookie is scoped to
 * `COOKIE_DOMAIN` (e.g. `.norien.live`) so it is shared across the marketing and
 * app subdomains. Returns null when Supabase is not configured so callers can
 * treat "signed out" and "auth disabled" the same way.
 */
