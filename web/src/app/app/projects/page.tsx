...
import { buildQuery, Input, intParam, Pagination, Toolbar } from '@/components/controls';
    ...
    <>
          ...
          columns={[
            {
              ...
              header: 'Project',
              cell: (project) => (
                <Link href={`/app/project/${project.slug}`} className="group inline-flex items-center gap-2.5">
                  {project.logo ? (
      ...
      <Pagination
        ...
        buildHref={(nextOffset) =>
          `/app/projects${buildQuery({ q, limit, offset: nextOffset }, { limit: 30, offset: 0 })}`
        }
      />
